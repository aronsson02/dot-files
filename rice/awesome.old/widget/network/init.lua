            ...
            awful.spawn.easy_async_with_shell(
                ...
                iw dev ]] .. interfaces.wlan_interface .. [[ link
                                ...
                                function(stdout)
                                        ...
                                        local bitrate = stdout:match('tx bithastighet: (.+/s)') or 'N/A'
                                        local message = 'Koblet til: <b>' .. (essid or 'Loading...*') ..
                                                '</b>\nTrådløst grensesnitt: <b>' .. interfaces.wlan_interface ..
                                                '</b>\nWiFi-styrke: <b>' .. tostring(wifi_strength) .. '%' ..
                                                '</b>\nBithastighet: <b>' .. tostring(bitrate) .. '</b>'

                                        if healthy then
                                                update_tooltip(message)
                                        else
                                                update_tooltip('<b>Tilkoblet, men ingen internett...</b>\n' .. message)
                                        end
        ...
        local update_wireless_icon = function(strength)
            awful.spawn.easy_async_with_shell(
                                ...
                                function(stdout)
                                        ...
                                        if not stdout:match('Tilkoblet, men ingen internett') then
    ...
    local update_wired = function()
        ...
        awful.spawn.easy_async_with_shell(
            ...
            function(stdout)
                                ...
                                if stdout:match('Tilkoblet, men ingen internett') then
                                        widget_icon_name = widget_icon_name .. '-alert'
                                        update_tooltip(
                                                '<b>Tilkoblet, men ingen internett...</b>' ..
                                                '\nEthernet-grensesnitt: <b>' .. interfaces.lan_interface .. '</b>'
                                        )
                                else
                                        update_tooltip('Ethernet-grensesnitt: <b>' .. interfaces.lan_interface .. '</b>')
                                        if startup or reconnect_startup then
                                                awesome.emit_signal('system::network_connected')
                                                notify_connected()
        ...
        elseif network_mode == 'wired' then
            ...
            if not reconnect_startup then
                ...
                update_tooltip('Nettverket er for øyeblikket koblet fra')
                widget.icon:set_image(widget_icon_dir .. widget_icon_name .. '.svg')
        end
