...
local notify_all_unread_email = function(email_data)
        ...
        local unread_counter = email_data:match('Ulest telling: (.-)Fra:'):sub(1, -2)

        local email_data = email_data:match('(Fra:.*)'):sub(1, -2)

        local title = nil

        if tonumber(unread_email_count) > 1 then
                title = 'Du har ' .. unread_counter .. ' uleste e-postmeldinger!'
        else
                title = 'Du har ' .. unread_counter .. ' uleste e-postmelding!'
        end
...
local notify_new_email = function(count, from, subject)
        if not startup_show and (tonumber(count) > tonumber(unread_email_count)) then
                ...
                local message = "Fra: " .. from ..
                "\nEmne: " .. subject

                naughty.notification ({
                        app_name = 'E-post',
                        title = 'Du har en ny ulest e-post!',
                        message = message,
                        timeout = 10,
                        icon = widget_icon_dir .. 'email-unread.svg'
        })
...
local set_invalid_credentials_msg = function()
        set_widget_markup(
                ...
                os.date('%d-%m-%Y %H:%M:%S'),
                'Du har ugyldig legitimasjon!'
        )
end

local set_latest_email_data = function(email_data)

        local unread_count = email_data:match('Ulest telling: (.-)Fra:'):sub(1, -2)
        local recent_from = email_data:match('Fra: (.-)Subject:'):sub(1, -2)
        local recent_subject = email_data:match('Emne: (.-)Lokal dato:'):sub(1, -2)
        local recent_date = email_data:match('Lokal dato: (.-)\n')
    ...
    awful.spawn.easy_async_with_shell(
        fetch_email_command,
                function(stdout)
                        ...
                        if stdout:match('Midlertidig feil i navneløsning') then
                                set_no_connection_msg()
                                return
                        elseif stdout:match('Ugyldige legitimasjon') then
                                set_invalid_credentials_msg()
                                return
                        elseif stdout:match('Ulest telling: 0') then
                                email_icon_widget.icon:set_image(widget_icon_dir .. 'email.svg')
                                set_empty_inbox_msg()
                                return
                        elseif not stdout:match('Ulest telling: (.-)Fra:') then
                                return
